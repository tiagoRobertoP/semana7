...
def gera_histograma_amigo_por_idade(gera_idade, qtde_usuarios_na_rede):
    xs = gera_idade
    ys = Counter(i for i, _ in gera_idade)
    plt.bar(xs, ys)
...
def idade(gera_idade, gera_sexo):
    idade = []
    sexo = []
    idadeESexo = []
    for i, x in gera_sexo:
            if(x == "M"):
                sexo.append((i,x))
    for i, x in gera_idade:
            if(x >=22):
                idade.append((i,x))
    for i, j in sexo:
        for x, y in idade:
            if (i==x):
                idadeESexo.append(y) 
    #print(idadeESexo)            
    a = Counter(i for i in idadeESexo)  
    #print (a)
    #print (Counter(x for x in a.values()))          
    return Counter(x for x in a.values())




def quantidade_masculino_22():
    print("Variância: ")
    print(variancia(idade(gera_idade(quantidade_de_usuarios_na_rede()), gera_sexo(quantidade_de_usuarios_na_rede()))))
    print("Desvio Padrão: ")
    print (desvio_padrao(idade(gera_idade(quantidade_de_usuarios_na_rede()), gera_sexo(quantidade_de_usuarios_na_rede()))))
...
def main():
    ...
    #gera_histograma_amigo_por_idade(gera_idade(quantidade_de_usuarios_na_rede()), quantidade_de_usuarios_na_rede())
    #idade(gera_idade(quantidade_de_usuarios_na_rede()), gera_sexo(quantidade_de_usuarios_na_rede()))
    quantidade_masculino_22()
    #idade(gera_idade(quantidade_de_usuarios_na_rede()), gera_sexo(quantidade_de_usuarios_na_rede()))
